;; WASIX command guest: exports its own memory, imports wasi proc_exit next to
;; wasmer_gpu_v0, and has a void _start. The bridge imports arrive through the
;; runtime's InstantiationHook, not from the runner. Input: 4 u32 at offset 0
;; (written by the host after instantiation). Output: 4 u32 at offset 16.
;; Exit code: 0 on success, otherwise the failing check number (bridge error
;; codes are negative and cannot be exit codes, so they map to 10 + step).
(module
  ...
  (func $_start (export "_start")
    ...
    (if (i32.lt_s (call $buffer_read (local.get $buffer) (i32.const 16) (i32.const 16)) (i32.const 0))
