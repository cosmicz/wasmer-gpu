;; Kernel guest: Collatz iteration counts (adapted from wasi-gfx-runtime
;; hello_compute). Input: 4 u32 at offset 0. Output: 4 u32 at offset 16.
;; The WGSL kernel is guest data at offset 4096; the host never sees it
;; before pipeline_create.
(module
  ...
  (func $_start (export "_start") (result i32)
    ...
    (local.set $status (call $buffer_read (local.get $buffer) (i32.const 16) (i32.const 16)))
    (if (i32.lt_s (local.get $status) (i32.const 0))
      (then (return (local.get $status)))
    )
