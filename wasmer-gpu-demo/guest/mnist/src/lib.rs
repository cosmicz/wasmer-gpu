//! MNIST softmax-regression trainer guest for the `wasmer_gpu_v0` bridge.
//!
...
//! own two WGSL kernels (`forward.wgsl`, `update.wgsl`). The host only feeds
...
//!
//! State buffer layout, in f32 elements (small read-back regions first so
//! `buffer_read`, which copies from offset 0, stays cheap):
//! `header(16) | loss(64) | probs(64x10) | W(10x784) | b(10) | X(64x784) | y(64)`.
#![no_std]
...
const PARAMS: usize = INPUTS * CLASSES + CLASSES;
const OFF_L: usize = 16;
const OFF_P: usize = OFF_L + MAX_BATCH;
const OFF_W: usize = OFF_P + MAX_BATCH * CLASSES;
const OFF_B: usize = OFF_W + INPUTS * CLASSES;
const OFF_X: usize = OFF_B + CLASSES;
const OFF_Y: usize = OFF_X + MAX_BATCH * INPUTS;
const TOTAL: usize = OFF_Y + MAX_BATCH;
const UPDATE_WORKGROUPS: usize = PARAMS.div_ceil(64);
...
static mut READBACK: [f32; OFF_B + CLASSES] = [0.0; OFF_B + CLASSES];
...
unsafe fn read_front(handle: i32, readback: &mut [f32; OFF_B + CLASSES], elements: usize) {
...
unsafe fn run() -> ! {
    ...
    // Seeded initialisation: small symmetric weights, zero biases.
    let mut rng = XorShift32(cfg.seed | 1);
    for k in 0..INPUTS * CLASSES {
        state[OFF_W + k] = rng.symmetric(0.01);
    }
    for j in 0..CLASSES {
        state[OFF_B + j] = 0.0;
    }
    ...
    report_weights(0, state.as_ptr().add(OFF_W), PARAMS as i32);

    let handle = buffer_upload(state.as_ptr(), (TOTAL * 4) as i32);
    check(handle, EXIT_UPLOAD);
    let forward = pipeline_create(FORWARD_WGSL.as_ptr(), FORWARD_WGSL.len() as i32);
    check(forward, EXIT_PIPELINE);
    let update = pipeline_create(UPDATE_WGSL.as_ptr(), UPDATE_WGSL.len() as i32);
    check(update, EXIT_PIPELINE);
    ...
    for step in 0..cfg.steps as usize {
        ...
        check(pipeline_dispatch(forward, handle, n as i32, 1, 1), EXIT_DISPATCH);
        check(pipeline_dispatch(update, handle, UPDATE_WORKGROUPS as i32, 1, 1), EXIT_DISPATCH);
        samples_seen += n;
        ...
        if step == 0 || progress_due || last {
            ...
            read_front(handle, readback, OFF_B + CLASSES);
            ...
            if step == 0 {
                report_weights(1, readback.as_ptr().add(OFF_W), PARAMS as i32);
            }
        }
    ...
    read_front(handle, readback, OFF_B + CLASSES);
    report_weights(2, readback.as_ptr().add(OFF_W), PARAMS as i32);
    check(pipeline_release(update), EXIT_RELEASE);
    check(pipeline_release(forward), EXIT_RELEASE);
